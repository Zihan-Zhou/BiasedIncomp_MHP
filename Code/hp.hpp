...
//		std::cout<<" exp: " <<expdist(rand)<<std::endl;
//		<<"fromi: "<<fromi<<" toi: "<<toi <<" fromt: "<<fromt
//		<< "currt: "<<currt <<" exp: " <<expdist(rand)
//		<<" addt: "<<kernel.invintphi(fromi,toi,expdist(rand)/2,currt-fromt)<< std::endl;
		return {toi,
	...
	struct gibbsstate {
		...
		gibbsstate(traj tr, double kk) : orig(std::move(tr)), kappa(kk) {
			curr.tend = orig.tend;
//			std::cout<<"gibbsstate sizes: "<<orig.events.size()<<" , "<<orig.unobs.size()<<std::endl;
...
//			std::cout<<"events.end " << bool(p==events.end()) <<" t: "<<t<<" p->first.t " << p->first.t<<std::endl;
